...
def test_qa_skill():
    skill = Skill()
    assert callable(skill.reply)
    replies = skill.reply('Hi', context='')
    assert len(replies) == 0
    obama_replies = skill.reply(
        'When was Obama born?',
...
def test_qa_skill_accuracy():
    skill = Skill()
    df = pd.DataFrame(get_bot_accuracies(skill))
    q_accuracies = df.groupby('question')['bot_accuracy'].max()
    mean_acc = q_accuracies.mean()
    log.warning(f'Mean skill accuracy: {mean_acc}')
    assert mean_acc > .4
